...
export RECALLNET_VLLM_MODEL="/mnt/ssd/recallnet/models/Meta-Llama-3.1-8B-Instruct"
export RECALLNET_MODEL_LIGHT="llama3.1:8b-fp8"
export RECALLNET_VLLM_MAX_LEN=4096
export RECALLNET_PROMPT="prompt_classify.txt"

cd ~/RecallNet/src/scripts

uv run vLLM_classify_mod.py \
    --dataset-prefix a \
    --predicates "at location" "capable of" "causes" "created by" "defined as" "desires" "distinct from" "has a" "has first subevent" "has last subevent" "has prerequisite" "has property" "has subevent" "manner of" "motivated by goal" "part of" "receives action" "used for"
