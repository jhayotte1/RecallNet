...
def parse_args():
    ...
    parser.add_argument("--exp-name", type=str, default="exp00_vllm", help="Experiment name")
...
def run_experiment(df: pd.DataFrame, experiment_name: str, experiment_desc: str, sample_size: int=100, pred: str="", pred_n_parsed:str=""):
    ...
    for batch, result in zip(batches, results):
        ...
        if len(sorted_evals) == len(batch):
            ...
            for idx_str, evaluation in result.evaluations.items():
                idx = int(idx_str)
                if idx >= len(batch):
                    print(f" Error on index generated by model: index: {idx} ; for batch of size {len(batch)} (pred={pred}). Triple skipped")
                    errors += 1
                    continue
                s, p, o = batch[idx]
                rows.append({
    ...
    print(f"Total: {total_time:.1f}s | Avg: {avg_time:.2f}s/triplet | Avg: {avg_time*1000:.2f}ms/triplet")
